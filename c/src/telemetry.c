...
//	buf = (unsigned char *)(frame->imageData);
	memcpy(buf, (unsigned char *)frame->imageData, frame->imageSize);
	return 0;
}
...
static int maxx=1280, maxy=720;

//static int maxx=720, maxy=1280;
...
static void dokey(rfbBool down,rfbKeySym key,rfbClientPtr cl)
{
  if(down) {
	...
	rfbMarkRectAsModified(cl->screen,0,0,1280,720);
  	cvSetCaptureProperty(cap, CV_CAP_PROP_FRAME_WIDTH, 640);
  	cvSetCaptureProperty(cap, CV_CAP_PROP_FRAME_HEIGHT, 480);

	
	capture((unsigned char *)(cl->screen->frameBuffer));
	rfbDrawString(cl->screen,&radonFont,100,500,"Samsung Demo!",0xffffff);
	rfbMarkRectAsModified(cl->screen,0,0,640,520);
	...
	initBuffer((unsigned char*)cl->screen->frameBuffer);
//	capture((unsigned char *)(cl->screen->frameBuffer));
	rfbDrawString(cl->screen,&radonFont,100,100,"Samsung Demo!",0x00ffff);
	rfbDrawString(cl->screen, &radonFont, 100, 150, "Press F2 to View Image Remotely", 0xff00ff);	
	rfbDrawString(cl->screen, &radonFont, 100, 180, "Press F3 to View Full Size Image Remotely", 0xffff00);
	rfbMarkRectAsModified(cl->screen, 0,0,1280,720);
	 ...
	 rfbMarkRectAsModified(cl->screen, 0, 0, 1280, 720);
      ...
      if (maxx < 1024) {
        if (maxx < 800) {
          ...
          newframebuffer(cl->screen, 1280, 720);
        }
      }
    } else if(key==XK_Down) {
      if (maxx > 640) {
...
int main(int argc,char** argv)
{
  ...
  if(!rfbScreen)
    return 0;
  rfbScreen->desktopName = "SAMSUNG Smart Machine";
  ...
  rfbDrawString(rfbScreen,&radonFont,20,100,"Samsung Demo!",0xffffff);

  /* This call creates a mask and then a cursor: */
   rfbScreen->cursor =
       rfbMakeXCursor(exampleXCursorWidth,exampleXCursorHeight,exampleXCursor,0);
 ...
 rfbRunEventLoop(rfbScreen,4000,FALSE);
